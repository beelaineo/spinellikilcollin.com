...
const getPreviewValues = async ({ label, link: previewLink }) => {
	if (!previewLink || !previewLink.document || !previewLink.document._ref) return { title: 'Missing Link' }
...
export const subMenu = {
	...
	preview: {
		...
		prepare: ({ title, columns }) => {
			const { linkGroup: linkGroups, imageBlock: imageBlocks } = groupBy(prop('_type'), columns || {})

			const subtitle = [
				linkGroups && linkGroups.length ? `${linkGroups.length} Link Group${linkGroups.length === 1 ? '' : 's'}` : undefined,
				imageBlocks && imageBlocks.length ? `${imageBlocks.length} Image Link${imageBlocks.length === 1 ? '' : 's'}` : undefined,
			]
				.filter(Boolean)
				.join(' | ')
			return {
