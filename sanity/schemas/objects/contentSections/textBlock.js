...
const getPreviewValues = async (values) => {
	...
	const linkedDoc = cta && cta.link && cta.link.document ? await getReferencedDocument(cta.link.document._ref) : undefined

	const info = [title, bodyText, linkedDoc ? `🔗${linkedDoc.title}` : null].filter(Boolean)
	...
	fields: [
		...
		},
		{
			...
			of: [
				{
					type: 'block',
					styles: [{ title: 'Normal', value: 'normal' }, { title: 'Header5', value: 'h5' }],
					marks: {
