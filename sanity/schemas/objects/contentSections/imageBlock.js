...
import { getImageThumbnail, getReferencedDocument, getShopifyThumbnail } from '../../utils'
...
const getPreviewValues = async (props) => {
	...
	const [imageUrl, linkedDoc] = await Promise.all([
		...
		link && link.document && link.document._ref ? getReferencedDocument(link.document._ref) : noop(),
	])
	...
	const info = [title, linkedDoc ? `🔗${linkedDoc.title}` : null, caption].filter(Boolean)
	const [previewTitle, ...subtitles] = info

	return {
		title: previewTitle,
		subtitles: [subtitles.length ? subtitles.join(' ') : undefined].filter(Boolean),
...
export const imageBlock = {
	...
	fields: [
		...
		},
		{
			...
			type: 'string',
			label: 'Title',
			description: "If emtpy, and if this content links to a page, product, or collection, that item's title will be used.",
		},
		{
			name: 'caption',
			type: 'string',
