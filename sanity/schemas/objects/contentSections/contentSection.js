...
const getPreviewValues = async (values) => {
	...
	const info = [
		...
		items && items.length ? `📝${items.length} item${items.length === 1 ? '' : 's'}` : null,
	].filter(Boolean)

	const titles = [blockValues.title, blockValues.subtitle, null, null].filter((i) => i !== undefined)

	const [title, subtitle] = titles
	return {
		title,
		subtitles: [subtitle, info.length ? info.join('  ') : undefined].filter(Boolean),
...
export const contentSection = {
	...
	fields: [
		...
		},
		{
			...
			type: 'string',
			options: {
				list: [{ title: 'Normal', value: 'normal' }, { title: 'Carousel', value: 'carousel' }],
				layout: 'radio',
				default: 'normal',
			},
