import { makeRemoteExecutableSchema, introspectSchema, mergeSchemas, transformSchema, RenameTypes } from 'graphql-tools'
...
const createSanitySchema = async () => {
	const sanityLink = new HttpLink({
		uri: `https://${config.get('sanity.projectId')}.api.sanity.io/v1/graphql/${config.get('sanity.dataset')}/default
  `,
		// Node-fetch type signatures do not match what apollo wants
		fetch: (fetch as unknown) as Fetch,
	})
...
export const createSchema = async () => {
	...
	const [shopify, sanity] = await Promise.all([createShopifySchema(), createSanitySchema()])
