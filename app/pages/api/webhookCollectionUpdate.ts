...
import { ShopifyGraphQLCollection } from './webhookRequestTypes'
...
export async function handleWebhookCollectionUpdate(
  client: SanityClient,
  collection: ShopifyGraphQLCollection,
): Promise<{
  ...
  const sortOrderUpper = (sort_order?.toUpperCase() || 'unknown').replace(
    '-',
    '_',
  )

  const shopifyId = collection.id

  // Fetch products
  const collectionProductsData = await fetchCollectionProducts(
    admin_graphql_api_id,
  )
  ...
  const collectionDocument: ShopifyDocumentCollection = {
    ...
    shopifyId: admin_graphql_api_id,
    store: {
      ...collection,
      id: shopifyId,
